...
def content1(transcript):
    ...
    completion1 = client.chat.completions.create(
        model="gpt-3.5-turbo",
        messages=[
        {"role": "system", "content": "You are a admitting internal medicine physician at a hospital. Please document using appropriate medical terminology and translate colloquial conversation into a format aprrporiate for formal medical documentation, including use of medical jargon, if needed. Provide a concise one liner including key elements of patient's past medical history and ending with their chief clinical complaint and the duration of the the chief complaint:"},
...
def content3(transcript):
    ...
    completion3 = client.chat.completions.create(
        model="gpt-3.5-turbo",
        messages=[
        {"role": "system", "content": "You are a admitting internal medicine physician at a hospital. Please document using appropriate medical terminology and translate colloquial conversation into a format aprrporiate for formal medical documentation, including use of medical jargon, if needed. Provide a short summmary of the key elements of the patients past medical history and history of present illness that you think would be the most helpful in generating differential clinical diagnoses. This list should be no longer than 10 items.:"},
        {"role": "user", "content": transcript}
        ]
    )
...
def content4(transcript):
    ...
    completion4 = client.chat.completions.create(
        model="gpt-3.5-turbo",
        messages=[
        {"role": "system", "content": "You are a admitting internal medicine physician at a hospital. Please document using appropriate medical terminology and translate colloquial conversation into a format aprrporiate for formal medical documentation, including use of medical jargon, if needed. Provide a clinical assessment based on the past medical history and history of present illness. This assessment should include a number of top clinical differentials in descending order of probability. It should also provide a preliminary assessment of the clinical prognosis for the patient and comment on their current clinical stability.:"},
        {"role": "user", "content": transcript}
        ]
    )
...
def content5(transcript):
    ...
    completion5 = client.chat.completions.create(
        model="gpt-3.5-turbo",
        messages=[
        {"role": "system", "content": "You are a admitting internal medicine physician at a hospital. Please document using appropriate medical terminology and translate colloquial conversation into a format aprrporiate for formal medical documentation, including use of medical jargon, if needed. Provide a Clinical Plan according to the clinical assessment above. The plan should be organized by clinical problem, in descending order of clinical significance and each problem's clincial plan should include several elements: a summary of the problem based on key elements of the history, consultant services that need to be contacted, specific diagnostics tests, imaging, and cultures that need to be ordered, and finally specific treatments, including surgical procedures, medications and calculate the specific dosages typically indicated for the condition. IF the dosaage is non-standard, please provide a weight-based dosage recommendation according to clinical and pharmacological guidelines."},
        {"role": "user", "content": transcript}
        ]
    )
